...
library(ecodist) # mixed models using distance matrices
library(geosphere) # to create the distance matrix
library(readxl) 

# Load datasets

spatial.var <- read_excel("SpatialModClimVar.xlsx") # spatial analysis 
...
MRM(dist(INBREEDING) ~ as.dist(D) + dist(Ndays35) + dist(Ndays5) + dist(Rain) + dist(Year), data=spatial.var, nperm=500)

MRM(as.dist(G) ~ as.dist(D) + dist(Ndays35) + dist(Ndays5) + dist(Rain) + dist(Year), data=spatial.var, nperm=500)
```
